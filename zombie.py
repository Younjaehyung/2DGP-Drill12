...
(1118, 274)]
    ...
    def move_slightly_mi(self, tx, ty):
        self.dir = math.atan2(ty-self.y, tx-self.x)
        distance = RUN_SPEED_PPS * game_framework.frame_time
        self.x -= distance * math.cos(self.dir)
        self.y -= distance * math.sin(self.dir)
    ...
    def set_random_location(self):
        self.tx,self.ty = random.randint(100,1280-100),random.randint(100,1200-100)
        return BehaviorTree.SUCCESS
        pass
    ...
    def run_to_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_mi(play_mode.boy.x,play_mode.boy.y)
        if self.distance_less_than(play_mode.boy.x,play_mode.boy.y,self.x ,self.y,r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('공 개수가 있는가?', self.is_ball)

        a4 = Action('소년한테 접근', self.move_to_boy)
        a5 = Action('소년한테 도망', self.run_to_boy)


        chase = Sequence('소년 추적', c2, a4)
        #run = Sequence('소년 추적', c1, run_or_chase)


        root = run_or_chase = Selector('추적 또는 도망', chase, a5)

        root = chase_boy = Sequence('소년 추적 거리', c1, run_or_chase)
        #root = run_boy = Sequence('소년을 추적', c1, a5)
